$(document).ready(function() {
    ...
    grid.click(function(item) {
        ...
        if ($(this).hasClass("gridActiv")) {
            ...
            TweenMax.to($(".preBtn"), 1, { x: windowWidth / -2 + 200, scale: 1, opacity: 1, ease: Elastic.easeOut.config(1, 0.3) })
            TweenMax.to($(".nextBtn"), 1, { x: windowWidth / 2 - 200, scale: 1, opacity: 1, ease: Elastic.easeOut.config(1, 0.3) })




            // TweenMax.to($(this), 0.3, { scale: scaleWidth, x: "+=" + centerX, y: "+=" + centerY, ease: Power2.easeIn })
            TweenMax.set($(this), { position: "absolute" })
            TweenMax.to($(this), 0.3, { left: "50%", top: "50%", xPercent: -50, yPercent: -50, ease: Power2.easeIn })
    ...
    // function towidth() {
    //     TweenMax.to(currentGrid, 5, { width: 720 })
    // }
